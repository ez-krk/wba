...
const vault = new PublicKey("HGxzts5edQeCuxmoeas9rEQkbsaYJuPmufdTYqJKXw8R");

(async () => {
  try {
    const lamports = await connection.getBalance(vault);
    console.log(`vault has ${lamports} lamports`);
    ...
    const tx = await program.methods
      .withdraw(new BN(lamports - rentExemptionAmount))
      .accounts({
