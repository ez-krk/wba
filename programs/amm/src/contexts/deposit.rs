...
use anchor_spl::{
    ...
    token_interface::{
        mint_to, transfer_checked, Mint, MintTo, TokenAccount, TokenInterface, Transfer,
    },
};
...
impl<'info> Deposit<'info> {
    pub fn deposit(
        ...
        max_x: u64,  // Max amount of X tokens to deposit
        max_y: u64,  // Max amount of Y tokens to deposit
        expiration: i64,
    ) -> Result<()> {
        ...
        let (x, y) =
            match self.mint_lp.supply == 0 && self.vault.amount == 0 && self.vault_y.amount == 0 {
                true => (max_x, max_y),
                false => {
    ...
    pub fn deposit_tokens(&self, is_x: bool, amount: u64) -> Result<()> {
        ...
        transfer_checked(ctx, accounts, self.mint_x.decimals)
    }
